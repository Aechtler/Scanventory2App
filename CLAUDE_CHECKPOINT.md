...
The next Batch 6 validation cleanup is now implemented on `scanapp2`, adding `npm run manual-regression:report` so each validation pass can print a dated report skeleton with branch/checklist metadata plus Trello handoff fields, and requiring that entrypoint in the documented quick checks.
...
The next backend route cleanup is now implemented on `scanapp2` for `packages/backend/src/routes/auth.ts`, keeping the public auth endpoints stable while moving credential normalization, password validation, and normalized auth error-envelope helpers into a focused sibling module with targeted Node coverage.
...
The next runnable-environment restore cleanup is now implemented on `scanapp2` for `scripts/workspace-toolchain-health.mjs`, allowing missing workspace packages to fall back to same-version locally installed package directories from known npm/global locations when the cache tarball is unavailable while still rejecting wrong-version local copies.
The next runnable-environment restore cleanup is now implemented on `scanapp2` for `scripts/workspace-toolchain-health.mjs`, extending the local fallback search to the OpenClaw workspace plus explicit extra search roots so pnpm-style installed packages can be reused when cache tarballs are missing; in this sandbox that now restores `typescript` during `npm run typecheck:backend`, reducing the direct backend blocker list from 13 to 12 packages.
The next runnable-environment diagnostics cleanup is now implemented on `scanapp2` for `scripts/setup-workspace-toolchain.mjs`, keeping workspace-scoped post-install failure output on direct blockers so `npm run typecheck:backend` reports the real 13 backend restore packages instead of exploding into 400+ ownerless hollow package misses after a failed offline workspace install.
...
The next runnable-environment diagnostics cleanup is now implemented on `scanapp2` for `scripts/workspace-toolchain-health.mjs`, making workspace-scoped failures recommend the matching guarded network retry (`SCANAPP_ALLOW_NETWORK_INSTALL=1 npm run typecheck:backend`) instead of bouncing back to repo-wide `setup:workspace` guidance while the same 13 uncached backend tarballs still block completion in this environment.
...
The next runnable-environment diagnostics cleanup is now implemented on `scanapp2` for `scripts/workspace-toolchain-health.mjs` and `scripts/setup-workspace-toolchain.mjs`, surfacing semver-compatible locally installed package candidates in failing backend/mobile restore output so the next manual or network-enabled restore pass can reuse concrete local versions when exact cache tarballs are absent.
...
### Aggregate backend build guard
- Added `scripts/run-workspace-build-all.mjs` as a small aggregate build runner that performs one shared `setup:workspace` preflight, then hands off to guarded per-workspace builds with `skipSetup`
- Updated `scripts/run-workspace-build.mjs` with the same `skipSetup` seam already used by the aggregate typecheck runner so shared setup does not run twice
- Retargeted root `build:all` to the guarded runner, added `scripts/run-workspace-build-all.test.ts`, and expanded `scripts/run-workspace-build.test.ts` plus `npm run test:targeted` so the aggregate build summary and skip-setup behavior stay covered in the dependency-limited workspace
...
- `node --test --experimental-strip-types packages/mobile/src/shared/components/Animated.test.ts`
  - Passed
- `node --test --experimental-strip-types scripts/run-workspace-build-all.test.ts scripts/run-workspace-build.test.ts`
  - Passed
- `npm run build:all`
  - Failed as expected in this sandbox because 13 backend tarballs remain uncached or hollow, but now stops at the guarded workspace-setup diagnostics and ends with `Workspace build summary: backend failed (exit 1)` instead of a raw `Cannot find module .../typescript/bin/tsc` crash
...
- `node ./scripts/setup-workspace-toolchain.mjs`
  ...
  - Unreadable cache blobs now count as misses too, so hollow packages like `typescript`, `tailwindcss`, and `tsx` no longer masquerade as locally restorable when their cached tarballs are empty or corrupt
...
- `npm run test:targeted`
  - Passed
- `node --test --experimental-strip-types scripts/workspace-toolchain-health.test.ts scripts/setup-workspace-toolchain.test.ts scripts/run-workspace-typecheck.test.ts`
  - Passed
...
- `node --test --experimental-strip-types scripts/workspace-toolchain-health.test.ts scripts/setup-workspace-toolchain.test.ts scripts/run-workspace-lint.test.ts scripts/run-workspace-typecheck.test.ts scripts/run-workspace-build.test.ts`
  - Passed
- `npm run typecheck:backend`
...
- `node --test --experimental-strip-types scripts/workspace-toolchain-health.test.ts scripts/setup-workspace-toolchain.test.ts`
  - Passed after adding a regression guard that missing packages can reuse same-version locally installed package directories when cache tarballs are absent, while still rejecting mismatched local versions
- `npm run typecheck:backend`
...
- `npm run typecheck:backend`
  - Still fails in this sandbox on the same 13 backend packages because no same-version local fallback copies are present; the new restore path is therefore truthful and no longer broadens the blocker set while remaining ready for cache-complete or globally preinstalled environments
...
- After the new local fallback restored `typescript`, the remaining direct backend `npm run typecheck:backend` blockers in this sandbox are 12 packages: `@prisma/client`, `@types/bcryptjs`, `@types/cors`, `@types/express-rate-limit`, `@types/multer`, `@types/uuid`, `bcryptjs`, `express-rate-limit`, `multer`, `prisma`, `tsx`, and `uuid`
- The guarded backend failure now also reports one semver-compatible local fallback candidate in this sandbox: `express-rate-limit@8.3.1` at `/usr/lib/node_modules/openclaw/node_modules/express-rate-limit`, which satisfies `@scanapp/backend`'s `^8.2.1` declaration but does not replace the remaining need to restore the exact lockfile-backed package set
- Use the guarded `npm run build:all` output to restore the backend build slice first when validating aggregate builds, since it now reports the same remaining direct backend blockers without dropping into raw TypeScript module-resolution noise
...
Use the guarded backend restore slice in a network-enabled environment to repopulate the remaining 12 backend tarballs first: `npm install --workspace=@scanapp/backend` (or `SCANAPP_ALLOW_NETWORK_INSTALL=1 npm run setup:workspace`), and if the environment stays sandboxed/offline, start from the newly reported local candidate for `express-rate-limit` before rerunning `npm run build:all` and `npm run typecheck:backend`.
