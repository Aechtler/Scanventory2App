...
- 2026-03-20: Runnable-environment-Diagnostik zeigt bei `npm run typecheck:backend` jetzt zusaetzlich semver-kompatible lokal installierte Fallback-Kandidaten aus den bestehenden Search-Roots an; `scripts/workspace-toolchain-health.test.ts` und `scripts/setup-workspace-toolchain.test.ts` guarden die neue Kandidaten-Sektion, und der reale Sandbox-Output weist nun konkret auf `express-rate-limit@8.3.1` unter `/usr/lib/node_modules/openclaw/node_modules/express-rate-limit` hin, waehrend die verbleibenden 12 Backend-Blocker unveraendert echt bleiben.
- 2026-03-20: Workspace-Restore durchsucht bei Cache-Miss jetzt zusaetzlich den lokalen OpenClaw-Workspace und weitere explizite Search-Roots nach pnpm-artig installierten Paketquellen; `scripts/workspace-toolchain-health.test.ts` guardet den neuen Search-Root-Pfad, und `npm run typecheck:backend` stellt dadurch `typescript` real wieder her, sodass der Backend-Blocker von 13 auf 12 Pakete sinkt.
